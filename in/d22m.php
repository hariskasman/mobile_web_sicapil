...
<?php include ("../sec/amenu.php"); ?>
		<h3 >(<?php echo $nm ; ?>) Monthly Log </h3>
		<img src="<?php echo $rsa ['pic']; ?>" alt="image" width="100px" height="100px">
		<br/>
		<input type="text" value="All : <?php echo $smloga; ?> Minute" readonly>
		<input type="text" value="Time : <?php echo $tla; ?>" readonly><br/>
		<button onclick="location='d22pm.php?u=<?php echo $rsa['codx'] ?>';" class="button">PRINT</button>
		<button onclick="location='d22pp.php?u=<?php echo $rsa['codx'] ?>';" class="button">PRINT PIC</button>
		<br/>
		...
		<?php
		$res = $link->query("SELECT * FROM log_book WHERE username='" .$nm. "' AND mt='" .$mt. "' ORDER BY id DESC");
